...
| **Decision questions** | The six questions (0-5) answered before recommending a change: already decided? · actual risk · existing self-control · cost to ordinary activity · supporting measurement, finished? · who watches it stays alive? Named without a count on purpose (`docs/01-orchestration.md`). |
| **Recommendation block** | The decision questions, a verification block and a "What I do NOT propose" section, injected verbatim into a delegation that must produce a recommendation — and only into those (`patterns/decision-questions/`). |
| **Verification block** | The top of a recommendation: what was measured and how, plus at least one verdict (`CONFIRMED` / `REFUTED` / `FALSE`) on a claim of the author's own. Without the verdict it is a reading list. |
| **Question-0 gate** | The coordinator's check on a returned recommendation: was a search run, and its result cited, even if empty? A failed return is relayed as failed, not hidden and not refused. |
| **Reach control (positive control)** | A known-present case found by the same probe, in the same run, from the same place. Without it, a negative result may mean the probe reached nothing. |
| **Negative control** | A neighbouring case that must give the opposite result. If both give the same, the probe does not discriminate. |
| **Probe that can fail** | A probe whose answer changes with its input, shown by a reach control and a negative control disagreeing in the same run. The alternative describes; it does not measure (`docs/15-verification.md`). |
| **Auditing by silences** | Looking for a verifier's false negatives among what it **passes** — historical cross-check and a baseline × configuration matrix — since analysis of its rejections cannot reveal them. |
| **Controls that contradict** | Two controls, each correct alone, giving opposite verdicts on one act. The defect lives between them and shows only when both are given the same fixture. |
| **Floor (harvest integrity)** | A collector's state when some sources or days were not harvested in full: the count is a lower bound, and a missing topic is uninformative. Opposite of **complete**. Distinct from a fault (`patterns/external-practice-radar/`). |
| **Mechanical digest** | A weekly agenda generated without a model — link, score, integrity marks, and an empty decision column — so the person forms the judgement rather than inheriting one. |
| **Adoption council / ledger** | The recurring human decision on each shortlisted practice (adopt · test · reject · park, with a reopening trigger for park), and the record of those decisions that later answers "did we already look at this?". |
| **Code-verification ladder** | Paper page → code-availability section → affiliation → organisation → scoped search; stops at the first hit and yields one of three exclusive labels. "Not verified" is forbidden. |
| **Domain manual** | The one document per domain with a duty to be right; the only place that holds paths, scripts and reasons. Other artefacts refer to it by manual and **section name** (`patterns/domain-manuals/`). |
| **Satellite manual** | A domain manual declared under a root index. A term shared between domains has one owning manual. |
| **Rollback net** | A verbatim backup or pre-patch copy kept beside a change. Drained after a dated go-ahead and a validation period, with the reason written into the domain's manual (`patterns/retiring-instruments/`). |
| **Cross-lineage red team** | A bounded Socratic dialogue (at most four rounds) with a model of another lineage, used when the coordinator is an interested party in a high-stakes judgement (`patterns/cross-lineage-red-team/`). |
| **Inconclusive by budget** | A red-team end state where rounds or quota ran out with objections still open. Never presented as "passed". |
| **Objection classifier** | Whoever decides whether an objection stands. Must not be the interested party; a specific, material, unresolved objection carries a mandatory consequence. |
| **Correction series** | An append-only log of the user's corrections, each tagged with a class (framing · memory · reading). A norm whose target class does not fall in about two months is redesigned, not reinforced. |
